...
def login():
    ...
    try:
        ...
        # --------------------------------------------
        return "<h2>Database Error</h2><p>An error occurred. Please try again later.</p>"
...
def open_redirect():
    ...
    # Security fix: Only allow relative URLs to prevent open redirect
...
def directory_traversal():
    ...
    try:
        # --------------------------------------------
        import os
        if not os.path.abspath(filename).startswith(os.path.abspath('.')):
...
def resource_leak():
    ...
    if not log_path.startswith(os.path.abspath('.')):
        return "Error: Invalid file path"
...
def ssrf_vulnerability():
    ...
    # SSRF protection: validate URL - restrict to safe external domains only
