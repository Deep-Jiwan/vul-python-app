...
def frame_content():
    ...
    # --------------------------------------------
    import html
    safe_frame_url = html.escape(frame_url, quote=True)
    html_content = f'''
        <h2>External Content</h2>
        <iframe src="{safe_frame_url}" width="800" height="600"></iframe>
    '''
    return render_template_string(html_content)
...
def open_redirect():
    ...
    # --------------------------------------------
    # Security fix: Only allow relative URLs (no domain) to prevent open redirect
...
def encrypt_data():
    ...
    return f"<h2>Encrypted Data:</h2><p>{encrypted.hex()}:{iv.hex()}</p>"
...
def ssrf_vulnerability():
    ...
    if parsed.scheme not in ['http', 'https'] or parsed.hostname not in ['example.com', 'localhost', '127.0.0.1']:
