...
def profile():
    # --------------------------------------------
    username = html.escape(request.args.get('name', session.get('username', 'Guest')), quote=True)
...
def encrypt_data():
    ...
    encrypted = iv + cipher.encrypt(padded_data.encode())
...
def resource_leak():
    ...
    if not log_path.startswith(safe_dir + os.sep):
