...
def profile():
    ...
    return render_template_string(template, username=username, comment=comment, password_hint=ADMIN_PASSWORD if username == 'admin' else '')
...
def resource_leak():
    ...
    safe_path = os.path.normpath(os.path.join(base_dir, log_file))
...
def xxe_vulnerability():
    ...
    try:
        ...
        return "Error parsing XML: An internal error occurred", 500
