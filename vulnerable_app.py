...
def login():
    ...
    try:
        ...
        if user:
            ...
            session['role'] = user[4]
            return f"<h2>Login successful! Welcome {html.escape(str(user[1]), quote=True)}</h2><br><a href='/profile'>View Profile</a>"
        else:
            # --------------------------------------------
        ...
        # --------------------------------------------
        return f"<h2>Database Error</h2><pre>{html.escape(str(e), quote=True)}</pre>"
...
def resource_leak():
    ...
    safe_dir = os.path.abspath('.') + os.sep
    if not abs_path.startswith(safe_dir):
        raise ValueError("Invalid path")
